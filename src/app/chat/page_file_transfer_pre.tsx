...
export default function ChatPage() {
  ...
  try {
    const token = localStorage.getItem('token');
    // Ensure URL matches @Controller('chat') path (without /api)
    ...
    if (res.ok) {
      const data = await res.json();
      // Normalize Prisma relation (msg.user.name) to flat property (userName)
  ...
  const joinRoom = (room: any) => {
    setCurrentRoom(room);
    setMessages([]);
    fetchRoomMessages(room.id); // <--- ADD THIS LINE
